...
def _ensure_dataset_yaml(path: Path) -> None:
    ...
    Ultralytics' trainer needs this file. We bootstrap when there isn't
    one yet so we don't overwrite a customised file.

    The ``path:`` field is written ABSOLUTE on purpose. Ultralytics
    resolves relative paths against its global ``datasets_dir`` (set in
    ``~/.config/Ultralytics/settings.json``), NOT against the yaml file
    — which on a fresh Linux/Pi install lands in the wrong place and
    raises a confusing ``Dataset 'training/dataset.yaml' images not
    found`` error.
    """
    if path.exists():
        return
    ...
    # Absolute path to data/ecoli so Ultralytics' datasets_dir can't
    # silently rebase it.
    dataset_root = (Path.cwd() / "data" / "ecoli").resolve()
    content = (
        "# Auto-generated by /api/train when training/dataset.yaml was missing.\n"
        "# `path:` is written absolute so Ultralytics' datasets_dir setting\n"
        "# doesn't relocate the dataset on Linux/Pi installs. Edit freely.\n\n"
        f"path: {dataset_root}\n"
        "train: images/train\n"
        "val: images/val\n"
        "test: images/test\n\n"
    ...
    log.info("Bootstrapped %s (path=%s) with classes=%s", path, dataset_root, classes)
