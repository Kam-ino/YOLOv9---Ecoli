...
#
# Once you've confirmed the app comes up, register it as a systemd
# service so it auto-starts on boot:
...
if [[ ! -x ".venv/bin/python" ]]; then
    if ! command -v python3 >/dev/null; then
        ...
        err "  sudo apt install -y python3 python3-venv python3-pip python3-full"
        exit 1
    fi
    log "Creating .venv ..."
    python3 -m venv .venv
...
# Some Ubuntu/Debian installs ship python3-venv without ensurepip's
# wheel cache, leaving the new venv pip-less. Try to bootstrap it; if
# that also fails, point at the apt packages they're missing.
if ! "$PYTHON" -m pip --version >/dev/null 2>&1; then
    log "pip is missing from .venv — bootstrapping via ensurepip..."
    if ! "$PYTHON" -m ensurepip --upgrade >/dev/null 2>&1; then
        err "Could not bootstrap pip into .venv."
        err "Fix:"
        err "  sudo apt install -y python3-pip python3-venv python3-full"
        err "  rm -rf .venv"
        err "  ./start.sh"
        exit 1
    fi
